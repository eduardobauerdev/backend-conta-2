...
async function upsertChat(chatData, metadata = {}) {
    ...
    // ⚠️ IMPORTANTE: NÃO enviamos 'name' nem 'last_message' no upsert
    // O nome é definido APENAS pelo usuário no frontend
    // A última mensagem é obtida via VIEW em tempo real
    const chatRecord = {
        id,
    ...
    // REMOVIDO: last_message - agora vem da VIEW
        ...
        sock.ev.on("messages.upsert", async ({ messages, type }) => {
            ...
            for (const msg of messages) {
                ...
                if (!chatData) {
                    ...
                    // Atualiza último timestamp (sem last_message)
                    const updateData = {
                        last_message_time: Number(msg.messageTimestamp) * 1000,
                        // REMOVIDO: last_message - agora vem da VIEW
