...
export async function POST(request: Request) {
  try {
    ...
    const callbackUrl = `${baseUrl}/api/callback/contrato-fisica`
    
    console.log("[v0] Enviando contrato física com callbackId:", callbackId)
    console.log("[v0] URL de callback completa:", callbackUrl)

    try {
      const response = await fetch("https://edubauerdev.app.n8n.cloud/webhook-test/contrato-fisica-inova-inox", {
        method: "POST",
        headers: {
