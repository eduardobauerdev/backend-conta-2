...
const SHEET_URL =
  "https://docs.google.com/spreadsheets/d/1EHtZXkMSjt_CCd1US6wVEJh4hyibPr4A7Or23qAAS2w/export?format=csv&gid=0"
    ...
    <div className="p-8 bg-background min-h-screen">
      <div className="max-w-6xl mx-auto space-y-6">
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-3xl font-bold text-foreground">Lista de Contratos</h1>
            <p className="text-muted-foreground mt-2">Visualize e gerencie todos os contratos</p>
            {error && (
          ...
          <div className="flex gap-2">
            ...
            <Link href="/gerar-documento">
              <Button className="flex gap-2 h-11 px-4 items-center">
                ...
                <span className="leading-none">Novo Contrato</span>
              </Button>
            </Link>
          </div>
        </div>
            ...
            <table className="w-full">
              <thead>
                <tr className="border-b-2 border-neutral-300">
                  ...
                  <th className="text-left py-3 px-4 font-semibold">Tipo de Projeto</th>
                ...
                {loading ? (
                    ...
                    <td colSpan={5} className="text-center py-12 text-muted-foreground">
                      <div className="space-y-2">
                        <p className="text-lg font-medium">
                          {searchTerm ? 'Nenhum contrato encontrado com os critérios de busca' : 'Nenhum contrato encontrado'}
                        </p>
                        <p className="text-sm">
                          {searchTerm ? (
                            ...
                            'Clique em "Novo Contrato" para criar seu primeiro contrato'
                          )}
                        </p>
                      </div>
                    </td>
                  ...
                  currentContratos.map((contrato, index) => (
                    <tr key={index} className="border-b border-neutral-200 hover:bg-muted/50">
                      ...
                      <td className="py-3 px-4">{contrato.tipoProjeto}</td>
                      <td className="py-3 px-4">{contrato.vendedor}</td>
                      <td className="py-3 px-4">{contrato.data}</td>
                      <td className="py-3 px-4">
                        {contrato.local && contrato.local.trim() !== '' ? (
                          <a
                            href={contrato.local}
                            target="_blank"
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center">
          ...
          <Card className="relative w-full max-w-md p-6 m-4 border-2 border-neutral-300 shadow-lg">
            <div className="flex items-center justify-between mb-4">
              <h2 className="text-xl font-bold">Buscar Contrato</h2>
              <Button
                variant="ghost"
                size="icon"
                className="h-8 w-8"
                ...
                <table className="w-full">
                  <thead>
                    <tr className="border-b-2 border-neutral-300">
                      ...
                      <th className="text-left py-2 px-3 font-semibold text-sm">Tipo de Projeto</th>
                  ...
                  <tbody>
                    {searchResults.map((contrato, index) => (
                      <tr key={index} className="border-b border-neutral-200 hover:bg-muted/50">
                        <td className="py-2 px-3 text-sm">{contrato.cliente}</td>
                        <td className="py-2 px-3 text-sm">{contrato.tipoProjeto}</td>
                        <td className="py-2 px-3 text-sm">{contrato.vendedor}</td>
                        <td className="py-2 px-3 text-sm">{contrato.data}</td>
