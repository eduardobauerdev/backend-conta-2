...
export function EditRoleDialog({ open, onOpenChange, role, onSuccess }: EditRoleDialogProps) {
  ...
  useEffect(() => {
    if (role && open) {
      ...
      const loadedPermissions: RolePermissions = { ...defaultPermissions }
      Object.keys(defaultPermissions).forEach((key) => {
        if (role[key] !== undefined) {
          loadedPermissions[key as keyof RolePermissions] = role[key]
        }
      })
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      console.log("[EditRoleDialog] Updating cargo:", cargoData)
      ...
      if (roleError) {
        console.error("[EditRoleDialog] Error updating role:", roleError)
        console.error("[EditRoleDialog] Error details:", JSON.stringify(roleError, null, 2))
      ...
      console.error("[EditRoleDialog] Error:", error)
      toast.error("Erro ao atualizar cargo")
    }
      ...
      <DialogContent className="sm:max-w-[700px] max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2">
            ...
            Editar Cargo: {role.nome}
          </DialogTitle>
          <DialogDescription>Atualize as permissões e configurações do cargo</DialogDescription>
        </DialogHeader>
          ...
          <div className="space-y-4">
            ...
            <div className="space-y-4">
              <div className="space-y-2">
                <Label htmlFor="nome">Nome do cargo (não editável)</Label>
                <Input
                  id="nome"
                  value={formData.nome}
                  disabled
          ...
          <div className="space-y-4">
            ...
            <div className="space-y-3">
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <div className="flex items-center space-x-2">
                  <Checkbox
                    id="can_edit_all_contracts"
                    checked={permissions.can_edit_all_contracts}
                    onCheckedChange={(checked) => updatePermission("can_edit_all_contracts", checked as boolean)}
                  />
                  <Label htmlFor="can_edit_all_contracts" className="text-sm font-normal cursor-pointer">
                    Editar contratos
                  </Label>
                </div>
                <div className="flex items-center space-x-2">
                ...
                <div className="flex items-center space-x-2">
                  <Checkbox
                    id="can_edit_all_orders"
                    checked={permissions.can_edit_all_orders}
                    onCheckedChange={(checked) => updatePermission("can_edit_all_orders", checked as boolean)}
                  />
                  <Label htmlFor="can_edit_all_orders" className="text-sm font-normal cursor-pointer">
